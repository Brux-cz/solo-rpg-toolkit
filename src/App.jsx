...
function EndSceneSheet({ onClose, cf, sceneNum, onCFChange, npcs, threads, onNpcsChange, onThreadsChange }) {
  ...
  const addThread = () => {
    ...
    if (!name || threads.length >= 25) return;
  ...
  const listItemStyle = { display: "flex", alignItems: "center", gap: 6, marginBottom: 4, padding: "5px 8px", border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 11, fontFamily: FONT };
    ...
    <Sheet title={`📕 KONEC SCÉNY ${sceneNum}`} onClose={onClose}>
      ...
      {npcs.map((n, i) => (
        <div key={i} style={listItemStyle}>
          ...
          <span style={{ fontSize: 10, color: C.muted }}>{n.weight}</span>
      ...
      {threads.map((t, i) => (
        <div key={i} style={listItemStyle}>
          ...
          <span style={{ fontSize: 10, color: C.muted }}>{t.progress}/{t.total}</span>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px" }}>
        ...
        {sub === "npc" && (
          <>
            <div style={{ fontSize: 9, color: C.muted, marginBottom: 6 }}>NPC SEZNAM ({npcs.length}/25)</div>
            {npcs.map((n, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, padding: "6px 10px", border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 11 }}>
                <span style={{ flex: 1 }}>{n.name}</span>
                <span style={{ fontSize: 10, color: C.muted }}>váha {n.weight}</span>
                <button onClick={() => onNpcsChange(npcs.filter((_, j) => j !== i))} style={delBtnStyle}>✕</button>
              </div>
            ))}
