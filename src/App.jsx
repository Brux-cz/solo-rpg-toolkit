  ...
  return (
    ...
    <div style={{ width: "100%", height: "100dvh", background: C.bg, display: "flex", flexDirection: "column", fontFamily: FONT, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflow: "hidden", maxHeight: sheetOpen ? "calc(50% - 10px)" : undefined }}>
        ...
        {tab === "world" && <SvetTab cf={game.cf} npcs={game.npcs || []} threads={game.threads || []} locations={game.locations || []} keyedScenes={game.keyedScenes || []} perilPoints={game.perilPoints || { aktualni: 2, max: 2 }} onGoToLobby={handleGoToLobby} onNpcsChange={(npcs) => updateGame({ npcs })} onThreadsChange={(threads) => updateGame({ threads })} onLocationsChange={(locations) => updateGame({ locations })} onKeyedScenesChange={(ks) => updateGame({ keyedScenes: ks })} onPerilPointsChange={(pp) => updateGame({ perilPoints: pp })} />}
      </div>
